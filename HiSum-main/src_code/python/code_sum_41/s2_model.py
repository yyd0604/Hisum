...
from torch_geometric.explain import Explainer, GraphMaskExplainer, GNNExplainer, PGExplainer
from torch_geometric.loader.data_list_loader import DataListLoader
from torch_geometric.utils import to_dense_batch
from torch_geometric.data.storage import (BaseStorage, NodeStorage,EdgeStorage)
from torch_geometric.nn.data_parallel import DataParallel
from torch_geometric.nn import HeteroConv, GraphNorm, HANConv, DirGNNConv, GCNConv, SAGEConv
import random
import numpy as np
import os
import logging
...
class CodeGraphEnc(nn.Module):
    def __init__(self,
        ...
        metadata = (['node'], [('node', 'base_child', 'node'),
                               ('node', 'base_father', 'node'),
        ...
        for _ in range(graph_gnn_layers):
            ...
            elif graph_GNN==HANConv:
                gnn = HANConv(emb_dims, emb_dims, heads=2,
                                dropout=0.6, metadata=metadata)
            elif graph_GNN == DirGNNConv:
                # conv = SAGEConv(emb_dims, emb_dims, aggr=graph_gnn_aggr)
                conv = GCNConv(emb_dims, emb_dims)
                gnn = DirGNNConv(conv=conv)
            else:
                gnn=HeteroConv({
    ...
    def forward(self, data):
        ...
        for gnn,relu,norm in zip(self.gnn_ops,self.grelu_ops,self.gnorm_ops):
            ...
            #如果是dirGCN
            output1 = gnn(x=data['node'].x,edge_index=data.edge_index_dict['node', 'base_child', 'node'])
            output2 = gnn(x=data['node'].x, edge_index=data.edge_index_dict['node', 'code_next', 'node'])

            # print(output)
            data['node'].x=norm(data['node'].x.add(relu(output1)).add(relu(output2)))
...
class TModel(TransSeq2Seq):
    ...
    def fit(self,
        ...
        # self.sentenceModel = SBert('roberta-large-nli-stsb-mean-tokens')  # 模型大小1.31G
        ...
        if self.train_mode:  # 如果进行训练
            ...
            for i in range(0,self.max_big_epochs):
                ...
                for j, batch_data in enumerate(pbar):
                    ...
                    pred_text_output_np = pred_text_output.detach().cpu().numpy()

                    text_dic = {'text_i2w': train_data['text_dic']['text_i2w'],
                                'ex_text_i2ws': [train_data['text_dic']['ex_text_i2ws'][k] for k in ids]}
                    # pred_text_output_np[:, self.tgt_begin_idx, :] = -np.inf  # (B,D_tgt,L_tgt)
                    # pred_text_output_np[:, self.pad_idx, :] = -np.inf  # (B,D_tgt,L_tgt)

                    # # 将decoder的输出转为句子
                    # sentences = self.generate_sentence_from_prob_matrix(pred_text_output_np, text_dic)
                    # sentences_lists = [' '.join(words) for words in sentences]
                    # # sentences_lists = [[' '.join(words)] for words in sentences]
                    # # print(sentences_lists)
                    #
                    # # 将原总结转为句子
                    # gold_text_sentence = self._tgt_ids2tokens(batch_text_output.to('cpu').data.numpy(), text_dic,
                    #                                           self.text_end_idx)
                    # # gold_text_sentence_lists = [[' '.join(words)] for words in gold_text_sentence]
                    # gold_text_sentence_lists = [' '.join(words) for words in gold_text_sentence]
                    # # print(gold_text_sentence_lists)
                    #
                    # # 使用sentence bert将两类句子进行编码
                    # encoded_pred_sentences = self.sentenceModel.encode(sentences_lists)
                    # encoded_gold_sentences = self.sentenceModel.encode(gold_text_sentence_lists)
                    # assert len(encoded_pred_sentences) == len(encoded_gold_sentences)
                    #
                    # # 计算余弦相似度
                    # # for index in range(len(encoded_pred_sentences)):
                    # #     cosine_similarity = util.cos_sim(encoded_pred_sentences[index], encoded_gold_sentences[index])
                    # #     similarities =similarities + cosine_similarity.item()
                    # cosine_scores = util.pytorch_cos_sim(encoded_pred_sentences, encoded_gold_sentences)
                    # similarities = torch.mean(cosine_scores).item()
                    # print(similarities)
                    #
                    # # sentence损失
                    # similarities = 0
                    # sentenceLoss = 1 - similarities
                    sentenceLoss = 0

                    loss = self.criterion(pred_text_output, batch_text_output) + sentenceLoss  # 计算loss
                    # if loss == math.nan:
                    #     print(j)
                    #     print(batch_data)
                    #     break
    ...
    def generate_sentence_from_prob_matrix(self, pred_text_output, text_dic, end_idx=0):

        word_indices = np.argmax(pred_text_output, axis=-1)  # shape: (192, 7160)

        # 将单词索引转换为句子序列
        sentences = []
        for indices in word_indices:
            # sentence = ' '.join([text_dic['text_i2w'][str(idx)] for idx in indices])
            #
            # sentences.append(sentence)
            sentence = []

            for j, text_ids in enumerate(indices):
                if text_ids != end_idx and j < 20:
                    text_i2w = {**text_dic['text_i2w'], **text_dic['ex_text_i2ws'][j]}
                    word = text_i2w[text_ids]
